       ...
       // private static MemCachedConfigInfo MemCachedConfigInfo = MemCachedConfigs.GetConfig();
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        if (MemCachedConfig.ApplyBase64)
                            val = UTF8Encoding.UTF8.GetBytes(ToBase64(Serialize(obj)));
            ...
            else
            {
                if (obj != null)
                {
                    ...
                    try
                    {
                        ...
                        val = Serialize(obj);//.GetBuffer();
                        if (MemCachedConfig.ApplyBase64)
                        {
                            val = UTF8Encoding.UTF8.GetBytes(ToBase64(val));
                        }
        ...
        private void LoadItems(SockIO sock, Hashtable hm, bool asString)
        {
            while (true)
            {
                    ...
                    if ((flag & F_SERIALIZED) == 0)
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                if (MemCachedConfig.ApplyBase64)
                                {
                                    buf = Base64ToBytes(UTF8Encoding.UTF8.GetString(buf, 0, length));
                                    o = Deserialize(buf);
                                }
                                else
                ...
                else if (END == line || ERROR == line || SERVER_ERROR == line || NOTFOUND == line)
                {
                    //if(log.IsDebugEnabled)
                    //{
        ...
        public Hashtable Stats(ArrayList servers, string command, string poolName)
        {
            ...
            if (string.IsNullOrEmpty(poolName))
                pool = SockIOPool.GetInstance(_poolName);
            else
        ...
        //private static ResourceManager _resourceManager = new ResourceManager("MemcachedLib.Memcached.StringMessages", typeof(MemcachedClient).Assembly);
