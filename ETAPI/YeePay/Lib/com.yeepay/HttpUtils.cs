    ...
    public abstract class HttpUtils
    {
        ...
        public static string SendRequest(string url, string para, string method)
        {
            ...
            if (method.ToUpper() == "POST")
            {
                ...
                {
                    int num2;
                    for (int i = 0; i < para.Length; i = num2 + 1)
                    {
                        num2 = para.IndexOfAny(anyOf, i);
                        if (num2 == -1)
                        {
                            builder.Append(HttpUtility.UrlEncode(para.Substring(i, para.Length - i), Encoding.GetEncoding("gb2312")));
                            break;
                        }
                        builder.Append(HttpUtility.UrlEncode(para.Substring(i, num2 - i), Encoding.GetEncoding("gb2312")));
                        builder.Append(para.Substring(num2, 1));
                    }
