...
namespace viviapi.ETAPI.Zweixin
{
    ...
    public class ZweixinPay : ETAPIBase
    {
        ...
        public string GetPayForm(string out_trade_no, decimal amount, string bankcode, bool autosumit, HttpContext Context)
        {
            ...
            string formUrl = "ZweixinCode.aspx";
            StringBuilder sbForms = new StringBuilder();
            sbForms.Append("<form id=\"form1\" action=\"" + formUrl + "\" methode=\"post\">");
            foreach (var strs in param.Split('&'))
            {
                var ary = strs.Split('=');
                string name = ary[0];
                string value = ary[1];
                sbForms.Append("<input type='hidden' name='" + name + "' value='" + value + "'/>");
            }
            sbForms.Append("</form>");
            if (autosumit == true)
            {
                sbForms.Append("<script type=\"text/javascript\" language=\"javascript\">setTimeout(\"document.getElementById('form1').submit();\",100);</script>");
            }
