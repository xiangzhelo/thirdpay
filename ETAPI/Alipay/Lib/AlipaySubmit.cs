...
namespace viviapi.ETAPI.Alipay.Lib
{
    /// <summary>
    /// 类名：Submit
    /// 功能：支付宝各接口请求提交类
    ...
    public class Submit
    {
        ...
        public static string BuildRequest(SortedDictionary<string, string> sParaTemp, string strMethod, string strButtonValue, bool autosumit)
        {
            //待请求参数数组
           ...
           // Model.SupplierInfo _suppInfo = Factory.GetCacheModel(101);

            string gateway = GATEWAY_NEW;

            //if (!string.IsNullOrEmpty(_suppInfo.jumpUrl))
            //{
            //    gateway = _suppInfo.jumpUrl + "/switch/alipay.aspx?";
            //}
            //else if (!string.IsNullOrEmpty(_suppInfo.postBankUrl))
            //{
            //    gateway = _suppInfo.postBankUrl;
            //}
