...
namespace viviAPI.WebUI2015
{
    ...
    public partial class Reg : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    DataTable table = DataBase.ExecuteDataset(CommandType.Text, "select * from emailCode where ID=" + id).Tables[0];
                    J_mobile.Value = table.Rows[0]["email"].ToString();
                    //J_mobile.Visible = false;
                    string cacheKey = string.Format(Constant.EmailRegCodeCacheKey, J_mobile.Value);
                    string validcode = (string)viviapi.Cache.WebCache.GetCacheService().RetrieveObject(cacheKey);
                    if (validcode != null)
                    {
                        if (code.ToUpper() == validcode.ToUpper())
                        {
                            J_mobile_code.Value = code;
                            //J_mobile_code.Attributes.Add("style","display:none;");
                            //T_reg_mobile_code2.Attributes.Add("style", "display:none;");
                        }
                    }
                    else
                        Page.ClientScript.RegisterStartupScript(Page.GetType(), "javascript"
                        , "<script language='javascript'>parent.window.location.href = '/reg.aspx';</script>");
        ...
        protected void iBtnSubmit1_Click(object sender, EventArgs e)
        {
            ...
            {
                pair = new KeyValuePair<string, string>("mobilecode", errMsg);
                ShowError(pair);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(errMsg))
            {
                pair = new KeyValuePair<string, string>("Span1", errMsg);
                ShowError(pair);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(errMsg))
            {
                pair = new KeyValuePair<string, string>("Span2", errMsg);
                ShowError(pair);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(errMsg))
            {
                pair = new KeyValuePair<string, string>("J_mobile_info", errMsg);
                ShowError(pair);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(errMsg))
            {
                pair = new KeyValuePair<string, string>("contact_name", errMsg);
                ShowError(pair);
                return;
            }
            if (string.IsNullOrEmpty(errMsg))
            ...
            if (!string.IsNullOrEmpty(errMsg))
            {
                pair = new KeyValuePair<string, string>("Span5", errMsg);
                ShowError(pair);
                return;
            }
            ...
            if (userId > 0)
            {
                ...
                //    AlertAndRedirect2(errMsg, "/login.aspx");
                //}
                //else
                //{
            ...
            {
                pair = new KeyValuePair<string, string>("Span7", "注册失败");
                ShowError(pair);
            }
        }
        #endregion
        ...
        void ShowError(KeyValuePair<string, string> pair)
        {
            HtmlGenericControl label = (HtmlGenericControl)this.FindControl(pair.Key);
            label.InnerHtml = string.Format("<i class=\"icon-money-tips\"></i>{0}", pair.Value);
        }
    }
