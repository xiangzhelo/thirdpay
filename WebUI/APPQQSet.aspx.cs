...
using viviLib.Web;
using UserLoginByPartner = viviapi.Model.User.UserLoginByPartner;
...
namespace viviAPI.WebUI7uka
{
    ...
    /// 
    /// </summary>
    public partial class APPQQSet : PageBase
    {
        ...
        public string openid
        {
            get
            {
                return WebBase.GetQueryStringString("openid", "");

            }
        }

        private UserLoginByPartner _model = null;
        public UserLoginByPartner model
        {
            get
            {
                if (_model == null && !string.IsNullOrEmpty(openid))
                {
                    _model = viviapi.BLL.User.UserLoginByPartner.Instance.GetModel(1, openid);
                }
                return _model;
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                if (!RegisterSettings.RegisterOpen)
                {
                ...
                var user = Session["QzoneOauth"] as QQUserInfo;
                if (user != null)
                {
                    litQQUserName.Text = user.nickname;
                }
        ...
        protected void iBtnSubmit_Click(object sender, ImageClickEventArgs e)
        {
            ...
            string password1 = Guid.NewGuid().ToString("N");
            string password2 = Guid.NewGuid().ToString("N");
